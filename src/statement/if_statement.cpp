...
QLang::IfStatement::IfStatement(const SourceLocation& where, StatementPtr if_, StatementPtr then, StatementPtr else_)
    : IfStatement(where, dyn_cast<Expression>(if_), std::move(then), std::move(else_))
{
}

QLang::IfStatement::IfStatement(const SourceLocation& where, ExpressionPtr if_, StatementPtr then, StatementPtr else_)
    : Statement(where), If(std::move(if_)), Then(std::move(then)), Else(std::move(else_))
{
}
...
void QLang::IfStatement::GenIRVoid(Builder& builder) const
{
    ...
    auto else_ = Else
                     ? llvm::BasicBlock::Create(builder.IRContext(), "else", parent)
                     : llvm::BasicBlock::Create(builder.IRContext(), "end", parent);
    const auto end = Else ? llvm::BasicBlock::Create(builder.IRContext(), "end", parent) : else_;
    ...
    else builder.IRBuilder().SetInsertPoint(end);
}
