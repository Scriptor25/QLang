...
QLang::ValuePtr QLang::GenMember(Builder& builder, ValuePtr object, const bool deref, const std::string& member)
{
    if (deref)
    {
    ...
    if (builder.IsCallee())
    {
        ...
        std::cerr << object->GetType() << " does not have a member with name '" << member << "'" << std::endl;
        return {};
    }
    ...
    return GenMember(builder, l_object, i);
}

QLang::LValuePtr QLang::GenMember(Builder& builder, const LValuePtr& object, const size_t i)
{
