...
QLang::ValuePtr QLang::GenInc(const SourceLocation& where, Builder& builder, const ValuePtr& value)
{
    ...
    case TypeId_Int:
        ...
        result = builder.IRBuilder().CreateAdd(value_ir, one_ir);
        break;
    case TypeId_Float:
        one_ir = llvm::ConstantFP::get(ir_ty, 1.0);
        builder.SetLoc(where);
        result = builder.IRBuilder().CreateFAdd(value_ir, one_ir);
        break;
